...
export class ApproachRingSystem {
    ...
    height = 0;
    // Configuration
    preemptTime = 1500; // ms before hit when ring starts
    maxScale = 4.0; // Ring starts at 4x key size
    noteCount = 3; // How many upcoming notes to show
    // Colors for proximity ramp
    farColor = '#ffffff'; // White for far notes
    midColor = '#00e5ff'; // Cyan for medium notes
    nearColor = '#76ff03'; // Green for near notes
    urgentColor = '#ffea00'; // Yellow for urgent notes
    // Judgment colors
    perfectColor = '#00e5ff';
    greatColor = '#76ff03';
    goodColor = '#ffea00';
    ...
    update() {
        ...
        for (const { note, timeUntilHit } of upcomingNotes) {
            ...
            if (this.rings.some(r => r.note === note))
                continue;
            // Skip if note is too far in the future
            if (timeUntilHit > this.preemptTime)
                continue;
...
//# sourceMappingURL=approach-ring-system.js.map