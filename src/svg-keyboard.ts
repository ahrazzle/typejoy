...
export class SVGKeyboardRenderer {
  ...
  private renderedKeys: Map<string, RenderedKey> = new Map();
  ...
  private renderKey(keyDef: KeyDef, x: number, y: number, w: number, h: number): RenderedKey {
  ...
  private getAriaLabel(keyDef: KeyDef): string {
  ...
  setNudgeGlow(keyId: string, intensity: number): void {
    const rendered = this.renderedKeys.get(keyId);
    if (!rendered) return;
    const bg = rendered.element.querySelector('.typejoy-keycap-bg') as SVGRectElement;
    if (bg) {
  ...
  setKeyHighlight(keyId: string, color: string, opacity: number = 0.6): void {
    const rendered = this.renderedKeys.get(keyId);
    if (!rendered) return;
    const bg = rendered.element.querySelector('.typejoy-keycap-bg') as SVGRectElement;
    if (bg) {
    ...
    const keycap = rendered.element.querySelector('.typejoy-keycap') as SVGRectElement;
    if (keycap) {
  ...
  applyTheme(theme: ThemeDescriptor, highContrast: boolean = false): void {
    this.theme = theme;
    ...
    // Update background
    const bg = this.svg.querySelector('.typejoy-keyboard-bg') as SVGRectElement;
    if (bg) bg.setAttribute('fill', colors.surface);

    // Update all keys
    for (const [, rendered] of this.renderedKeys) {
      const keycap = rendered.element.querySelector('.typejoy-keycap') as SVGRectElement;
      const text = rendered.element.querySelector('.typejoy-keycap-text') as SVGTextElement;
      if (keycap) {
